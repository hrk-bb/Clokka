...
| S-08 | 社員作成・更新・無効化 | `GET/POST/PATCH /admin/employees` | 同画面の一覧を更新し、操作結果を表示。 | APIのリクエスト/レスポンス項目が未定のため、最終実装前にAPI契約を確定する。 |
...
| S-08 社員・部署管理 | 社員一覧、氏名、社員ID、部署、ロール、有効状態、作成/編集/無効化を表示する。無効化は履歴を削除しないことを確認表示する。初期パスワード・再設定、部署マスタの取得/更新、社員APIのリクエスト形式が未定のため、これらの入力フォーム詳細はAPI契約確定後に固定する。 |
