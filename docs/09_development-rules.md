...
| 状態 | レビュー待ち — Q-04承認待ち |
| 最終更新日 | 2026-08-29 |
| 依存関係 | `../AGENTS.md`（運用原則の正本）、`08_directory.md`、`01_requirements.md`〜`07_security.md`、`03_tech-stack.md` |

## 1. 基本原則

運用原則の正本は`../AGENTS.md`である。承認ゲート、無断変更の禁止、矛盾の検出・報告、秘密情報の扱いといった原則の詳細は重複記載せず、必ず`AGENTS.md`を参照すること。本章では、それらの原則をコード実装の文脈に適用する際の補足のみを記す。

- Q-04承認前にPhase 6のコードを開始しない（`AGENTS.md`原則3の適用）。
- 契約（要件・ADR・API・DDL）の不変条件を変更する必要がある場合は`Q-xx`を起票し、`decisions.md`にADRを追記する（`AGENTS.md`原則2・5の適用）。
- コミットは小さく単一目的に保つ。ドキュメントとコードの混在は`chore: normalize structure`のような場合を除き行わない（`AGENTS.md`「変更とGitのルール」の適用）。
