# Clokka アーキテクチャ設計（Phase 2 承認済み版）
...
| Render Free Postgresの失効 | データ消失 | 永続DBにはNeon Free PostgreSQLを使用する。実社員データのMVP利用は`D-06`で承認済みであり、HTTPS、認可、監査ログ、Secrets管理を必須とする。 |
| クラウド移行時のコスト増 | 予算超過 | クラウド選定時に月額上限、監視、停止手順を承認する。AWSは無料前提にしない。 |
| 単一Renderサービスの停止 | 一時的に全機能停止 | MVPではRender Freeの制約として受容する。本番移行時に冗長化、監視、バックアップ、復元手順、RTO/RPOを運用文書で検証する。 |
| Push拒否 | 能動リマインドが届かない | 管理者ページで許可・拒否・未設定・非対応・未確認を一覧・絞込みできるようにし、拒否者を把握する。 |
| 公開アプリケーションまたはSecretsの侵害 | 個人情報漏えい | HTTPS、最小権限、Secrets分離、認可、監査ログ、バックアップ暗号化を実装・運用で確認する。 |

## 7. 現在の次工程

本構成と`03_tech-stack.md`はPhase 2で承認済みである。現在はPhase 3としてDB、API、画面、セキュリティの詳細設計をレビューしている。Phase 3の全体承認までは、アプリケーションコードやクラウド環境を作成しない。
