...
| KI-009 | Resolved in the Phase 3 review design on 2026-08-26. `notification_deliveries` reserves a logical employee notification before sending, with `UNIQUE(employee_id, notification_date)` using the job's JST date; `notification_delivery_attempts` records subscription-level outcomes. The chosen at-most-once behavior may miss a Push after a crash, but does not issue a second Push request for the same employee/date. |
